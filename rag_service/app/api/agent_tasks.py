...
async def respond_to_agent_task_result_review(
    ...
    task = await _owned_task(task_id, thread_id)
    run = await AgentWorkflowRepository().get_run(req.run_id)
    ...
    try:
        ...
        linked_run = None
        if req.decision == "retry_with_input":
            existing_linked = next(
                (
                    candidate for candidate in await repository.list_task_runs(task.id)
                    if candidate.parent_run_id == req.run_id
                    and (candidate.run_metadata_json or {}).get("result_review_idempotency_key") == idempotency_key
                ),
                None,
            )
            linked_run = existing_linked or await ensure_task_run(task.id)
            task = await repository.get_task(task.id) or task
        return {
