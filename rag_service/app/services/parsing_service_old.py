...
def extract_text_with_coordinates(data: bytes, filename: str):
    ...
    try:
        ...
        # Write raw Docling output to test folder
        test_dir = "/app/tests"
        os.makedirs(test_dir, exist_ok=True)
        
        # Write original string format
        test_file_str = os.path.join(test_dir, f"docling_raw_output_{filename}.json")
        try:
            with open(test_file_str, "w") as f:
                json.dump(str(docling_result), f, indent=2)
            logger.info(f"Raw Docling output (string format) written to {test_file_str}")
        except Exception as write_err:
            logger.warning(f"Failed to write Docling output to file: {write_err}")
        
        # Write JSON format using export_to_dict()
        ...
        try:
            ...
            logger.info(f"Raw Docling output (dict format) written to {test_file_json}")
        except Exception as dict_err:
            logger.warning(f"Failed to write Docling dict output to file: {dict_err}")
