...
test('document workspace tabs include browser, documents, and debug trace', () => {
  ...
  assert.deepEqual(tabs.map((tab) => tab.kind), ['browser', 'document', 'trace']);
  assert.equal(tabs[0].id, 'browser-tab');
  assert.equal(tabs[1].id, 'file-1');
  assert.equal(tabs[2].id, 'trace-tab');
  assert.equal(tabs[2].status, 'running');
